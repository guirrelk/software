...
//用户头像地址
const imageUrl = ref(managerInfoStore.info.avatarUrl)
//头像修改
import {managerAvatarUpdateService} from '@/api/manager.js'
...
const uploadSuccess = async (result) =>{
    ...
    imageUrl.value = result.data;
        //调用接口
    let resultMsg = await managerAvatarUpdateService(imageUrl.value);

    ElMessage.success(resultMsg.msg? resultMsg.msg:'修改成功')

    //修改pinia中的数据
    managerInfoStore.info.avatarUrl = imageUrl.value;
}
